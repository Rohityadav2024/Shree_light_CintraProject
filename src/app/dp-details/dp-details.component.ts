import { Component, OnInit } from '@angular/core';
  ...
  import { Directory,Industry } from '../bridge';
...
@Component({
  ...
  styleUrls: ['./dp-details.component.scss']
})
export class DPDetailsComponent implements OnInit {
      ...
      constructor(private modalService: NgbModal,
        private router: ActivatedRoute, private route: Router, public bridgeService2: BridgeService) { }

      ngOnInit(): void {
        this.bridgeService2.autoCall();
        this.idd = this.router.snapshot.params.id;
        this.getIndustryList(this.idd);
      ...
      getIndustryList(id:any): void {
        this.isLoading2 = true;
        this.bridgeService2.getDirectoryPagination(this.pagination,this.searchValue,this.order_by_field,this.order_by_value,id).subscribe(
          (data: any) => {
      ...
      suplier(item: any) {
        // this.route.navigate(['/competitors/competitors-details/' + item]);
      }
  ...
  onFileChanged(event: any) {
    ...
    for (var i = 0; i < event.target.files.length; i++) {
      ...
      addIndustry(f: NgForm) {
        this.indus.DirectoryGroup = this.idd;


    if (this.fl) {
      this.indus.Attachment = this.fl;
    }
    else {
      this.indus.Attachment = '';
    }
        ...
        if (f.valid) {
          this.isLoading = true;
          this.bridgeService2.insertDirectorypartner(this.indus,this.isEdit).subscribe(
            (res: Directory) => {
