...
import { Segment } from '../bridge';
import { BridgeService } from '../modules/service/bridge.service';
declare var $: any;


@Component({
  selector: 'app-customer-segment-master',
  templateUrl: './customer-segment-master.component.html',
  styleUrls: ['./customer-segment-master.component.scss']
})
export class CustomerSegmentMasterComponent implements OnInit {
  p: number = 1;
  sortedColumn: string = '';
  ...
  getIndustryList(): void {
    ...
    this.bridgeService2.getSegmentByPagination(this.pagination,this.searchValue,this.order_by_field,this.order_by_value).subscribe(
      (data: any) => {
  ...
  addIndustry(f: NgForm) {
    ...
    if (f.valid) {
      this.isLoading = true;
      this.bridgeService2.insertSegment(this.indus,this.isEdit).subscribe(
        (res: Segment) => {
