...
export class PriceListComponent implements OnInit {
  ...
  getIndustryList(): void {
    ...
    this.bridgeService2.getPricelistByPagination(this.pagination,this.searchValue,this.order_by_field,this.order_by_value).subscribe(
      (data: any) => {
  ...
  addIndustry(f: NgForm) {
    ...
    if (f.valid) {
      this.isLoading = true;
      this.bridgeService2.insertPrice(this.indus,this.isEdit).subscribe(
        (res: PriceList) => {
