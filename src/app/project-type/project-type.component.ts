...
export class ProjectTypeComponent implements OnInit {
  ...
  getIndustryList(): void {
    ...
    this.bridgeService2.getProjectByPagination(this.pagination,this.searchValue,this.order_by_field,this.order_by_value).subscribe(
      (data: any) => {
  ...
  addIndustry(f: NgForm) {
    ...
    if (f.valid) {
      this.isLoading = true;
      this.bridgeService2.insertProject(this.indus,this.isEdit).subscribe(
        (res: PrjectType) => {
