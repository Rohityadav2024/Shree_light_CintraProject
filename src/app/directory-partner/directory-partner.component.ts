...
declare var $: any;
@Component({
  selector: 'app-directory-partner',
  templateUrl: './directory-partner.component.html',
  styleUrls: ['./directory-partner.component.scss']
})
export class DirectoryPartnerComponent implements OnInit {
    ...
    getIndustryList(): void {
      ...
      this.bridgeService2.getDirectoryByPagination(this.pagination,this.searchValue,this.order_by_field,this.order_by_value).subscribe(
        (data: any) => {
    ...
    suplier(item: any) {
      this.route.navigate(['/directory-partner/dp-details/' + item]);
    }
    ...
    addIndustry(f: NgForm) {
      ...
      if (f.valid) {
        this.isLoading = true;
        this.bridgeService2.insertDirectorPartner(this.indus,this.isEdit).subscribe(
          (res: Industry) => {
